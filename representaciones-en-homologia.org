...
\begin{tikzpicture}
  ...
  \draw (-90:1cm) node [fill=orange!80!white,below]{$\Delta_{1}$};
\end{tikzpicture}
    #+end_src
...
\begin{tikzpicture}
  ...
  \draw (-90:1cm) node [fill=orange!80!white,below]{$\Delta_{2}$};
\end{tikzpicture}    
     #+end_src
...
\begin{tikzpicture}
    ...
    (210:1) node {} --
    (0,0);
  \draw (0,-1) node [fill=orange!80!white,below]{gráfica $G$};
  \filldraw[fill=blue!50!white] (4,0) node[circle,left]{$d$} --
   (5,0.5) node[above]{$a$} --
   (5,-0.5) node[below]{$c$} -- cycle;
   \draw (5,0.5) -- (6,0)  node[right]{$b$} -- (5,-0.5);
   \draw (5,-1) node [fill=purple!80!white,below]{complejo $\mathcal{N}(G)$};
\end{tikzpicture}
     #+end_src
...
***                                                         :B_ignoreheading:
    ...
    sucesión de espacios vectoriales \(C_{k}(\Delta)=0\) junto con
    transformaciones lineales \(d_{k}\colon C_{k}(\Delta)\to
    C_{k-1}(\Delta)\), tales que \(d_{k}\circ d_{k+1}=0\)
