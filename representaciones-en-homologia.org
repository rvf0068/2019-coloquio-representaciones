...
*** Ejemplos de grupos					     :B_exampleblock:
    ...
    - \(\mathrm{GL}(V)\) :: grupo de las transformaciones lineales
         ...
         general lineal*)
    - \(M_{n}(F)\) :: grupo de matrices cuadradas invertibles
                      \(n\times n\) con entradas en un campo
                      \(F\). (*grupo de matrices*)
...
*** Correspodencia                                                :B_theorem:
    ...
    #+header: :cache yes
    #+begin_src latex :results raw file
\begin{tikzpicture}[every node/.style={draw,minimum height=2em,very thick,align=center,text width=3cm,top color=white, bottom color=black!20}]
  \graph[grow right sep=2cm]{$S_{n}$ módulos irreducibles <->[very thick]clases de conjugación de $S_{n}$};
\end{tikzpicture}
    #+end_src
